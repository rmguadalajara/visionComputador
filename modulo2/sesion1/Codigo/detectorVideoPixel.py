...
out = cv2.VideoWriter('outputPixel.avi', fourcc, fps, (frame_width, frame_height),True)
...
def pixelado(img, rects, tam_bloque):
    for x1, y1, x2, y2 in rects:
        roi = img[y1:y2, x1:x2]
        #recorrer la imagen por bloques de tamaño tam_bloque
        for i in range(0, roi.shape[0], tam_bloque):
            for j in range(0, roi.shape[1], tam_bloque):
                #calcular el color medio del bloque
                color = np.mean(roi[i:i+tam_bloque, j:j+tam_bloque], axis=(0,1))
                #pintar el bloque con el color medio
                roi[i:i+tam_bloque, j:j+tam_bloque] = color
        img[y1:y2, x1:x2] = roi
    return img
